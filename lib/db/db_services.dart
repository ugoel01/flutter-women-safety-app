...
class DatabaseHelper {
  ...
  Future<TContact?> getContactByPhoneNumber(String phoneNumber) async {
    Database db = await this.database;
    List<Map<String, dynamic>> result = await db.query(
      contactTable,
      where: '$colContactNumber = ?',
      whereArgs: [phoneNumber],
    );

    if (result.isNotEmpty) {
      return TContact.fromMapObject(result.first); // Assuming TContact has a fromMapObject constructor
    } else {
